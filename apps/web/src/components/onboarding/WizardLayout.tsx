...
import { useBriefOnboardingComplete } from "@/lib/auth";
...
export function WizardLayout({ config }: Props) {
  ...
  const briefComplete = useBriefOnboardingComplete();
  ...
  const handleNext = async () => {
    ...
    if (isLastStep) {
      ...
      try {
        const { user } = await briefComplete.mutateAsync({ name, contact });
        ...
        if (err instanceof Error && err.message === "BRIEF_EXISTING_EMAIL") {
          toast.error("Аккаунт с этим email уже есть — войдите");
