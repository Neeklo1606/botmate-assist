...
import { isAppAuthGuardEnabled } from "@/lib/auth";
import { fetchCurrentUser } from "@/lib/auth";
import { isAuthTransientError } from "@/lib/auth/api-auth-service";
import { useCurrentUser, useLogout } from "@/lib/auth";
import { useCabinetRealtime } from "@/lib/realtime/use-cabinet-realtime";
...
export const Route = createFileRoute("/_app")({
  beforeLoad: async ({ context, location }) => {
    if (!isAppAuthGuardEnabled()) {
      return;
    }

    let user: Awaited<ReturnType<typeof fetchCurrentUser>> = null;
    try {
      ...
      // Сеть/сервер недоступны — НЕ редиректим на /login (предотвращаем петлю
      // при кратковременном даунтайме API). Пропускаем рендер: UI покажет
      // «Связь восстанавливается» и `useCurrentUser` дождётся ответа.
      if (isAuthTransientError(err)) {
        return;
      }
      throw err;
    }
...
const RUNTIME_NAV: NavItem[] =
  runtimeTenantUiEnabled() ?
    runtimeWorkspaceUiEnabled() ?
      [
        { to: "/runtime", label: "Исполнения", icon: Activity },
        { to: "/runtime/workspace", label: "Раб. пространство", icon: Layers },
        { to: "/runtime/operator", label: "Оператор", icon: Binoculars },
        { to: "/runtime/consistency", label: "Согласованность", icon: AlertTriangle },
        { to: "/runtime/compare", label: "Сравнение", icon: GitCompare },
      ]
    : [{ to: "/runtime", label: "Исполнения", icon: Activity }]
  : [];
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="hidden text-sm md:block" style={{ color: "rgba(255,255,255,0.55)" }}>
          {user?.workspaceName ?? "Рабочая область"}
        </div>
      </div>
